...
<div style="padding: 30px 20px; text-align: center; color: #65676b;">
    ...
    <div>No results found for "<%= keyword != null ? keyword.replaceAll("<", "&lt;").replaceAll(">", "&gt;") : "" %>"
    </div>
</div>
<% } else { %>
...
<div class="search-result-item" onclick="goToProfile(<%= user.getId() %>)">
    <div style="width: 40px; height: 40px; border-radius: 50%; background: #e4e6ea; display: flex; align-items: center; justify-content: center; margin-right: 12px; color: #65676b;">
        ...
        <img src="${pageContext.request.contextPath}/static/images/<%=avatar%>"
             style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover;"
             alt="<%= fullname.replaceAll("\"", "&quot;") %>">
        <% } else { %>
        <i class="fas fa-user"></i>
        <% } %>
    </div>
    <div style="flex-grow: 1;">
        <div style="font-weight: 600; color: #1c1e21; font-size: 14px;"><%= fullname.replaceAll("<", "&lt;").replaceAll(">", "&gt;") %>
        </div>
        <div style="color: #65676b; font-size: 13px;">@<%= username.replaceAll("<", "&lt;").replaceAll(">", "&gt;") %>
        </div>
    </div>
</div>
<% } %>
...
<div class="search-result-item" onclick="goToPost(<%= post.getPostId() %>)">
    ...
    <div style="flex-grow: 1;">
        ...
        <div style="color: #1c1e21; font-size: 13px; line-height: 1.3;"><%= content.replaceAll("<", "&lt;").replaceAll(">", "&gt;") %>
        </div>
    </div>
</div>
<% } %>
...
<div class="search-result-item" onclick="goToGroup(<%= group.getId() %>)">
    ...
    <div style="flex-grow: 1;">
        <div style="font-weight: 600; color: #1c1e21; font-size: 14px;"><%= groupName.replaceAll("<", "&lt;").replaceAll(">", "&gt;") %>
        </div>
        <% if (description != null && !description.trim().isEmpty()) { %>
        <div style="color: #65676b; font-size: 13px;"><%= description.replaceAll("<", "&lt;").replaceAll(">", "&gt;") %>
        </div>
        <% } %>
    </div>
</div>
